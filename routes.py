...
def hermit_page(name, rarity):
    ...
    cur.execute(f"""SELECT * FROM Card WHERE Name = '{name}'
                 AND Rarity = '{rarity}';""")
    ...
    # extract certain stats for calculating card strength/rating
    ...
    cur.execute(f"""SELECT ID, Name, Rarity FROM Card WHERE ID IN
                 (SELECT ID FROM Hermit WHERE OwnerName='{HermitStats[0][2]}')
                 AND NOT ID = {id};""")
    ...
    return render_template('hermit_details.html',
                           HeadStats=HeadStats, HermitStats=HermitStats,
                           SeeAlso=SeeAlso, title=title, Header=Header,
                           stylesheet=stylesheet, A1Worded=A1Worded,
                           A2Worded=A2Worded, CAV=CAV, Rulings=Rulings,
                           clashwinfixed=clashwinfixed,
...
def search(category, query):
    ...
    return render_template('search.html', Header=Header, title=Title,
                           stylesheet=stylesheet, Cards=Cards,
                           category=category)
...
def rulebook():
    ...
    return render_template('rules.html', title=Title, Header=Header,
                           stylesheet=stylesheet)
...
def about():
    ...
    return render_template('about.html', title=Title, Header=Header,
                           stylesheet=stylesheet)
...
def changes():
    ...
    return render_template('changes.html', title=Title, Header=Header,
                           stylesheet=stylesheet)
